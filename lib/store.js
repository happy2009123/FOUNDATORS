...
import { createPost, toggleLikePost, addComment as addPostComment, updateComment as updatePostComment, deleteComment as removePostComment, toggleCommentLike as toggleCommentLikeFS, toggleFollowUser as toggleFollowFS, sendMessage as sendChatMessage, bookmarkPost, recordEvent, createChat, markNotificationRead, markAllNotificationsRead as markAllNotificationsReadFS } from './firestore';
import { notifyUser } from './notify';
import { db } from './firebase';
import { doc, setDoc, serverTimestamp } from 'firebase/firestore';
import { checkRateLimit } from './rateLimit';
import { initialsAvatar } from './avatar';
import { stopPresenceHeartbeat, resetPresenceWatchers } from './presence';
    ...
    (set, get) => ({
      ...
      login: (userData) => {
        const profile = {
          ...
          handle: userData.handle || '@' + (userData.displayName || 'user').toLowerCase().replace(/\s+/g, ''),
      ...
      publishPost: async ({ text, tagType, imageUrl }) => {
        if (!checkRateLimit('post', 5, 60000).allowed) {
          get().showToast('Slow down! Try again in a minute.');
          return null;
        }
        ...
        if (!res.success) {
          // Roll back the optimistic post so a failed write is never faked
          // as a published post.
          set((s) => {
            const { [id]: _removed, ...commentsByPost } = s.commentsByPost;
            return { posts: s.posts.filter((post) => post.id !== id), commentsByPost };
          });
          get().showToast(`Could not publish post: ${res.error || 'unknown error'}`);
          return null;
        }
        recordEvent('post_created', { postId: id, authorKey: p.id }).catch(() => {});
        return id;
      },
      ...
      toggleLike: (postId) => {
        ...
        set((s) => {
          ...
          // Store false (not delete) so UI fallbacks can tell "explicitly
          // unliked" apart from "never hydrated".
          nextLiked[postId] = !isLiked;
        ...
        recordEvent('post_liked', { postId, userId: get().profile.id }).catch(() => {});
        if (nowLiked) {
          const post = get().posts.find((p) => p.id === postId);
          if (post && post.authorKey !== get().profile.id) {
            notifyUser(post.authorKey, {
              type: 'like',
              actorKey: get().profile.id,
              actorName: get().profile.name,
              text: `${get().profile.name} liked your post`,
              linkType: 'post',
              linkId: postId,
            }).catch(() => {});
          }
      ...
      addComment: (postId, text, replyTo) => {
        if (!checkRateLimit('comment', 10, 60000).allowed) {
          get().showToast('Slow down! Try again in a minute.');
          return;
        }
        const p = get().profile;
        ...
        recordEvent('comment_created', { postId, authorKey: p.id }).catch(() => {});
        const commentPost = get().posts.find((ps) => ps.id === postId);
        if (commentPost && commentPost.authorKey !== p.id) {
          notifyUser(commentPost.authorKey, {
            type: 'comment',
            actorKey: p.id,
            actorName: p.name,
            text: `${p.name} commented on your post`,
            linkType: 'post',
            linkId: postId,
          }).catch(() => {});
        }
      ...
      editComment: (postId, commentId, newText) => {
        ...
        updatePostComment(postId, commentId, newText).catch(() => {});
      },
      ...
      deleteComment: (postId, commentId) => {
        ...
        removePostComment(postId, commentId).catch(() => {});
      },
      ...
      toggleCommentLike: (postId, commentId) => {
        ...
        toggleCommentLikeFS(postId, commentId, get().profile.id).catch(() => {});
      },
      ...
      toggleFollowUser: (key) => {
        ...
        if (key === me) {
          ...
          get().showToast('You cannot follow yourself');
          return;
        }
        toggleFollowFS(me, key).catch(() => {});
        recordEvent('user_followed', { followerId: me, followedId: key }).catch(() => {});
        if (nowFollowing) {
          notifyUser(key, {
            type: 'follow',
            actorKey: me,
            actorName: get().profile.name,
            text: `${get().profile.name} started following you`,
            linkType: 'profile',
            linkId: me,
          }).catch(() => {});
        }
      ...
      contacts: {},
      unreadByContact: {},
      // uid -> lastSeen epoch ms (or null). Hydrated live by usePresence();
      // cleared on logout so the previous account's partners never linger.
      presence: {},
      ...
      sendMessage: (contactKey, text) => {
        const p = get().profile;
        const contact = get().contacts[contactKey];
        if (!contact) return;
        ...
        set((s) => ({
          contacts: {
            ...
            [contactKey]: {
              ...contact,
              messages: [...(s.contacts[contactKey]?.messages || []), { id: makeId('msg'), who: p.id, text, time: 'Now' }],
            },
          },
        }));

        if (contact.chatId) {
          sendChatMessage(contact.chatId, { text, senderKey: p.id, senderName: p.name, senderAvatar: p.avatar }).catch(() => {});
          // Notify recipient
          notifyUser(contactKey, {
            type: 'message',
            actorKey: p.id,
            actorName: p.name,
            text: `${p.name}: ${text.substring(0, 100)}`,
            linkType: 'message',
            linkId: contact.chatId,
          }).catch(() => {});
        } else {
          createChat({
            participants: [p.id, contactKey],
            participantNames: { [p.id]: p.name, [contactKey]: contact.name },
            participantAvatars: { [p.id]: p.avatar, [contactKey]: contact.avatar },
            ...
            if (result.success) {
              ...
              set((s) => ({
                contacts: {
                  ...s.contacts,
                  [contactKey]: { ...s.contacts[contactKey], chatId },
                },
              }));
              sendChatMessage(chatId, { text, senderKey: p.id, senderName: p.name, senderAvatar: p.avatar }).catch(() => {});
            }
          }).catch(() => {});
        }
      },
      ...
      startOrFindConversation: (query) => {
        ...
        set((s) => ({
          contacts: {
            ...
            [key]: {
              name: query.trim(),
              avatar: initialsAvatar(query.trim()),
              online: false,
              status: 'Offline',
              lastActive: '',
              messages: [],
      ...
      markNotificationsRead: (id) => {
        ...
        markNotificationRead(get().profile.id, id).catch(() => {});
      },
      ...
      markAllNotificationsRead: () => {
        ...
        markAllNotificationsReadFS(get().profile.id).catch(() => {});
      },
      ...
      blockUser: (userKey) => {
        ...
        if (db && get().blockedUsers[userKey]) {
          setDoc(doc(db, 'users', get().profile.id, 'blocked', userKey), {
      ...
      reportItem: (item) => {
        ...
        if (db) {
          const reportId = `${get().profile.id}_${item.targetUserId || 'unknown'}_${Date.now()}`;
          setDoc(doc(db, 'reports', reportId), {
            reporterId: get().profile.id,
            reportedId: item.targetUserId || null,
            reason: item.reason || '',
