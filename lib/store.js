...
import { createPost, toggleLikePost, addComment as addPostComment, toggleFollowUser as toggleFollowFS, sendMessage as sendChatMessage, bookmarkPost, createNotification, recordEvent, createChat } from './firestore';
    ...
    (set, get) => ({
      ...
      toggleLike: (postId) => {
        ...
        if (nowLiked) {
          ...
          if (post && post.authorKey !== get().profile.id) {
            createNotification(post.authorKey, {
              type: 'like',
      ...
      addComment: (postId, text, replyTo) => {
        ...
        if (commentPost && commentPost.authorKey !== p.id) {
          createNotification(commentPost.authorKey, {
            type: 'comment',
            actorKey: p.id,
            actorName: p.name,
