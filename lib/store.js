...
import { createPost, toggleLikePost, addComment as addPostComment, toggleFollowUser as toggleFollowFS, sendMessage as sendChatMessage, bookmarkPost, recordEvent, createChat } from './firestore';
  ...
  persist(
    (set, get) => ({
      ...
      toggleFollowUser: (key) => {
        ...
        recordEvent('user_followed', { followerId: get().profile.id, followedId: key }).catch(() => {});
        if (nowFollowing) {
          notifyUser(key, {
            type: 'follow',
            actorKey: get().profile.id,
