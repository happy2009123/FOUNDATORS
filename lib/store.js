...
function seedLikedBookmarked(posts) {
  const liked = {};
  const bookmarked = {};
  posts.forEach((p) => {
    if (p.likedDefault) liked[p.id] = true;
    if (p.bookmarkedDefault) bookmarked[p.id] = true;
  });
  return { liked, bookmarked };
}

const { liked: seedLiked, bookmarked: seedBookmarked } = seedLikedBookmarked(INITIAL_POSTS);
...
export const useStore = create(
  persist(
    (set, get) => ({
      ...
      login: () => set({ isLoggedIn: true, sessionExpiry: Date.now() + 24 * 60 * 60 * 1000 }),
      logout: () => {
        set({
          isLoggedIn: false,
          sessionExpiry: null,
          profile: { ...KABIR },
          posts: INITIAL_POSTS,
          likedPosts: seedLiked,
          bookmarkedPosts: seedBookmarked,
          commentsByPost: INITIAL_COMMENTS,
          ...
          followedUsers: {},
          followedStartups: {},
          unreadByContact: { sophia: 3, daniel: 1, community: 5 },
        });
      },
          ...
          return {
            discussions: {
              ...
              [discId]: {
                ...
                comments: [...s.discussions[discId].comments, { who: 'me', text }],
              },
            },
          };
        });
        ...
        set((s) => {
          ...
          return {
            contacts: {
              ...
              [contactKey]: {
                ...contact,
                messages: [...contact.messages, { id: makeId('msg'), who: 'out', text, time: 'Now' }],
              },
            },
          };
        });
  ...
  return (
      ...
      signCollab: (collabId, { name, message }) => {
        set((s) => ({
          collabGestures: s.collabGestures.map((c) =>
            c.id === collabId
              ? { ...c, signatures: [...c.signatures, { authorKey: 'kabir', name, message, createdAt: new Date().toISOString().split('T')[0] }] }
              : c
          ),
        }));
      },
    ...
    }),
    {
      ...
      partialize: (state) => {
        const {
          toastMessage, toastId, isDrawerOpen,
          login, logout, updateBio, updateProfile, publishPost, toggleLike, toggleBookmark,
          addComment, addDiscussionComment, toggleFollowUser, toggleFollowStartup,
          sendMessage, attachFile, startOrFindConversation, ensureContactForUser,
          markAllNotificationsRead, unreadNotificationCount, toggleSetting,
          showToast, openDrawer, closeDrawer, markContactRead, toggleTheme,
          blockUser, reportItem,
          createGesture, reactToGesture, incrementGestureViews,
          publishTemplate, forkTemplate, starTemplate,
          createCollab, signCollab, closeCollab,
          toggleFollowCreator, creatorStats, followedCreators,
          viewedGestures,
          recordRemix, likeComment, addTemplateComment, toggleFeatured, recordTemplateView,
          getTemplateAnalytics, setTemplatePrice, togglePremium, recordEarning, getCreatorEarnings,
          premiumTemplates, templatePrice, creatorEarnings,
          templateRemixes, templateComments, featuredTemplates, templateAnalytics,
          joinedEvents, toggleJoinEvent,
          challengeProgress, startChallenge, completeChallenge,
          ideaVotes, toggleIdeaVote,
          uploadedFiles, uploadFile, getUploadedFile,
          ...rest
        } = state;
        return rest;
      },
